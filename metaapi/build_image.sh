...
tar -xzf *.tgz
echo '解压tag包成功'
#mv "./bkdata/${SUBMODULE}api/Dockerfile" ./
mv "./bkdata/${SUBMODULE}api" ./code
mv ./bkdata/support-files ./support-files

VERSION=$(cat ./code/VERSION)

# 镜像编译
docker buildx build --platform linux/amd64  . -t "bkbase-${SUBMODULE}api:${VERSION}-${BUILD_NO}"
echo 'docker镜像构建成功'
...
docker tag "bkbase-${SUBMODULE}api:${VERSION}-${BUILD_NO}" $bkbase_docker_repository_domain/bkbase/bkbase-docker/bkbase-${SUBMODULE}api:$docker_version
